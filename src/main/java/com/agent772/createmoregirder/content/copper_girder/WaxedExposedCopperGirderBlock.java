...
public class WaxedExposedCopperGirderBlock extends CopperGirderBlock {
    ...
    public WeatheringCopper.WeatherState getAge() {
        return WeatheringCopper.WeatherState.EXPOSED; // Waxed exposed copper stage
    }
    ...
    protected ItemInteractionResult useItemOn(ItemStack stack, BlockState state, Level level, BlockPos pos, Player player, InteractionHand hand, BlockHitResult hitResult) {
        ...
        if (AllBlocks.SHAFT.isIn(stack)) {
            KineticBlockEntity.switchToBlockState(level, pos, CMGBlocks.WAXED_EXPOSED_COPPER_GIRDER_ENCASED_SHAFT.getDefaultState()
                    .setValue(WATERLOGGED, state.getValue(WATERLOGGED))
                    .setValue(TOP, state.getValue(TOP))
                    .setValue(BOTTOM, state.getValue(BOTTOM))
