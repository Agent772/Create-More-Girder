 ...
 *
 * Interaction matrix:
 *   - Sneak + right-click host that has our bracket attached:
 *       cleared mimic → returned to player.
 *   - Right-click host with valid block in offhand:
 *       first attach captures mimic from offhand.
 *       repeated click with same block cycles the mimic texture orientation.
 *   - Otherwise behaves as a normal {@link BracketBlockItem}.
 */
public class CopycatBracketBlockItem extends BracketBlockItem {
    ...
    public @NotNull InteractionResult useOn(UseOnContext context) {
        ...
        // Sneak-clear: only if we already have a copycat bracket attached
        ...
        // If our bracket is already attached, intercept to handle apply/cycle from offhand
        if (beh != null && beh.isBracketPresent() && isOurBracket(beh.getBracket())
            ...
            if (candidate != null) {
                ...
                // mimic already present and differs from offhand: do nothing (don't overwrite)
                return InteractionResult.SUCCESS;
            }
            // No valid offhand material — fall through to BracketBlockItem so the user
            // can keep right-clicking to (no-op) re-attach the same bracket.
        }

        // Capture state before placement so we can detect a fresh attach
        ...
        if (!level.isClientSide && result == InteractionResult.SUCCESS && !hadBracketBefore) {
            // Re-resolve behaviour after placement; same host position
